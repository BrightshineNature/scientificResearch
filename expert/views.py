...
def homeView(request):
    ...
    re_list_1 = Re_Project_Expert.objects.filter(Q(expert = expert) & Q(is_first_round = True))
    re_list_2 = Re_Project_Expert.objects.filter(Q(expert = expert) & Q(is_first_round = False))
    context = {}

    context.update(getContext(re_list_1, 1, "item", 0, 2))
    context.update(getContext(re_list_2, 1, "item2", 0, 2))
...
def applicationView(request):
    ...
    score_table = forms.BasicScientificResearchScoreTable.objects.get(re_obj = re_obj)
    project_info_form = ProjectInfoForm()
    basis_content_form = BasisContentForm()
    base_condition_form = BaseConditionForm()

    if request.method == "GET":
        score_form = forms.BasicScientificResearchScoreForm(instance = score_table)
        context = {
        ...
        re_id = request.GET.get("re_id")
        score_form = forms.BasicScientificResearchScoreForm(request.POST, instance = score_table)
